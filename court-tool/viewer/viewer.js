...
async function loadTranscript() {
  ...
  if (!r.ok) {
    document.body.innerHTML = `<div style="padding:40px;font-family:monospace">
      Failed to load transcript: ${r.status}<br>
      Make sure outputs/merged.json exists. Run:<br>
      <code>python transcribe_deepgram.py audio.mp3 && python merge.py --primary outputs/deepgram.json</code>
    </div>`;
...
function loadSpeakerNames() {
  try {
    ...
    const key = state.transcript.audio;
    state.speakerNames = all[key] || {};
  } catch { state.speakerNames = {}; }
}
function saveSpeakerNames() {
  const all = JSON.parse(localStorage.getItem(state.storageKey) || "{}");
  all[state.transcript.audio] = state.speakerNames;
  localStorage.setItem(state.storageKey, JSON.stringify(all));
}

function assignSpeakerColors() {
  const ids = [...new Set(state.transcript.segments.map(s => s.speaker))];
  ids.sort();
  ids.forEach((id, i) => {
    state.speakerColors[id] = SPEAKER_PALETTE[i % SPEAKER_PALETTE.length];
  });
}
...
function initWaveform() {
  ...
  state.ws.on("audioprocess", onAudioProcess);
  ...
  $("#speakerList").innerHTML = ids.map(id => `
    <div class="speaker-row" data-id="${id}">
      ...
      <span class="speaker-name">
        <input type="text" data-spk="${id}" value="${speakerLabel(id)}"
               placeholder="说话人 ${id}">
      </span>
    </div>
  `).join("");
...
function renderReviewJumps() {
  ...
  state.transcript.segments.forEach(seg => {
    const reviewCount = seg.words.filter(w => w.needs_review).length;
    if (reviewCount > 0) {
      jumps.push({ id: seg.id, time: seg.start, count: reviewCount });
    }
  });
  $("#reviewJump").innerHTML = jumps.slice(0, 50).map(j => `
    <button class="jumplink" data-time="${j.time}" data-seg="${j.id}">
      ${fmtTime(j.time)} · ⚠ ${j.count}
    </button>
  `).join("") || `<div style="font-size:12px;color:var(--ink-mute)">无</div>`;
...
function renderTranscript() {
  ...
  const html = segs.map(seg => `
    <div class="segment" id="seg-${seg.id}" data-seg="${seg.id}">
      ...
      <div class="seg-text" data-seg="${seg.id}">
        ${seg.words.map((w, i) =>
...
function bindTranscriptEvents() {
  ...
  $$(".btn-translate").forEach(el => {
    el.onclick = () => {
      const segId = parseInt(el.dataset.seg);
      openTranslatePanel(segId);
    };
  });
}

function openPopover(evt, word, seg) {
  const pop = $("#popover");
...
async function openTranslatePanel(segId) {
  ...
  if (next && next.speaker !== seg.speaker && next.start - seg.end < 8) {
    ...
    humanBody.textContent = "（未自动定位到法庭翻译员的对应段落）";
    humanBody.classList.remove("loading");
  }
...
function updateMeta() {
  ...
  $("#engineLabel").textContent =
    `主: ${m.primary_engine}` + (m.compared_engines?.length ? ` · 比对: ${m.compared_engines.join(", ")}` : "");
  $("#reviewLabel").textContent =
    `待核 ${m.stats.review_words}/${m.stats.total_words}`;
}
