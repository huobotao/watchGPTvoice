...
`tictactoe/` 是一个独立小项目 — 穷举井字棋 255,168 局对弈并可视化。直接用浏览器打开 `tictactoe/index.html`：

- **博弈树浏览器**：从空盘开始，每个空格标注下在这里之后 X 胜 / O 胜 / 平 的局数，点格子下钻。
- **统计仪表盘**：结局占比、首手落点命运热力图、对局长度分布。
- **全景放射图**：把整棵博弈树压缩到一张圆盘上，色彩区分 X / O / 平 主导的子树，点圈块直接跳进浏览器。
