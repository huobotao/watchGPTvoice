...
const COLS = {
  ...
  session:   ["conversation", "talker", "chatusrname", "session", "chat", "chat_id", "room", "session_id", "username"],
  sender:    ["sender", "fromuser", "sender_name", "speaker", "from"],
  type:      ["mestype", "type", "msg_type", "messagetype"],
  msgid:     ["mesid", "mes_id", "msg_id", "id", "rowid", "mesvolume", "mes_local_id", "meslocalid", "local_id", "server_id"],
};
...
function normalizeDirection(v, columnName) {
  ...
  const s = lc(v);
  // String variants
  if (s === "self" || s === "me" || s === "true" || s === "out" || s === "send" || s === "sent") return true;
  if (s === "other" || s === "false" || s === "in" || s === "recv" || s === "received") return false;
  const n = Number(v);
...
// Detect Codex's WCDB/zstd unresolved-content marker
const UNRESOLVED_RE = /^\[WCDB\/zstd[^\]]*\]/;
function isUnresolvedContent(s) {
  return typeof s === "string" && UNRESOLVED_RE.test(s);
}
...
function renderMessageRow(m) {
  ...
  const unresolved = isUnresolvedContent(m.content);
  const bubbleClass = `msg-bubble type-${m.type}` + (unresolved ? " unresolved" : "");
  row.innerHTML = `
...
function renderStats() {
  ...
  const unresolvedCount = msgs.filter(m => isUnresolvedContent(m.content)).length;
  ...
  body.innerHTML = `
    <div class="stats-grid">
      ...
      ${unresolvedCount > 0 ? `<div class="stat-card" style="grid-column: 1/-1; background: #fffbeb; border-left: 3px solid #f59e0b;"><div class="label">⚠ WCDB 字典未解(dict_id=5)</div><div class="value" style="font-size:16px;">${unresolvedCount.toLocaleString()} 条 — 需要 .sqlite-first.material 字典文件才能还原</div></div>` : ""}
    </div>
